...
// Extern variables from .ino
extern camera_fb_t *motion_fb;
extern size_t motion_size;
extern unsigned long motion_capture_time;
extern bool motion_available;
...
static esp_err_t motion_handler(httpd_req_t *req) {
  ...
  if (!motion_available || motion_fb == NULL) {
    httpd_resp_send(req, "No motion image yet", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
  }
  ...
  return httpd_resp_send(req,
                         (const char *)motion_fb->buf,
                         motion_fb->len);
}

static esp_err_t motion_stats_handler(httpd_req_t *req) {
  ...
  if (!motion_available) {
    ...
    snprintf(json, sizeof(json),
             "{\"image_size\":%u,\"capture_time_ms\":%lu}",
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_uri_t motion_stats_uri = {
      .uri = "/motion_stats",
      .method = HTTP_GET,
      .handler = motion_stats_handler,
      .user_ctx = NULL
    };
    ...
    httpd_register_uri_handler(camera_httpd, &motion_stats_uri);
